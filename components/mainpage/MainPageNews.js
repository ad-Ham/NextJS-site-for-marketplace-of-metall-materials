...
export const MainPageNews = ({ news, newsHot, user }) => {
	...
	const newsCards = news.slice(0, 2).map(newsObj => {
		...
		<Card p="sm" key={news.id} shadow="xl" style={{ marginBottom: '10px', minHeight: '275px' }}>
			...
			<Grid justify={"center"}>
				<Grid.Col span={4}>
					<Image src={'data:image/'+newsObj.photopath.substr(newsObj.photopath.length-3)+';base64,'+ newsObj.image} height={100} layout="fill" />
				</Grid.Col>
				<Grid.Col span={8}>
		...
		<Card p="sm" key={news.id} shadow="xl" style={{ marginBottom: '10px', minHeight: '185px' }}>
				...
				<Group position="apart" style={{ marginBottom: 5, marginTop: theme.spacing.sm }}>
					<Link href={'/news/'+newsObj.id} passHref><Title order={3} weight={300} style={{fontSize:18}}>{newsObj.title}</Title></Link>
				</Group>
		</Card>
		</MediaQuery>
		</>);
	...
	const bottomNews = news.slice(2,6).map(newsObj => {
		...
		<MediaQuery largerThan="sm" styles={{ display: 'none' }} key={'0' + newsObj.id}>
